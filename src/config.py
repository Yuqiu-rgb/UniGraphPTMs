...
def get_train_config():
    ...
    parse.add_argument('--path', type=str, default=None,
                       help='specify the path for the model to evaluate')
    parse.add_argument('-learn-name', type=str, default='BERT_validation', help='learn name')
    parse.add_argument('-save-best', type=bool, default=True,
                       help='if save parameters of the current best model ')
    ...
    parse.add_argument('-num-head', type=int, default=8,
                       help='number of head in multi-head attention')  # 8
    # parse.add_argument('-dim-embedding', type=int, default=128, help='residue embedding dimension')  # 64
    parse.add_argument('-dim-embedding', type=int, default=1024,
                       help='residue embedding dimension')  # 64
    parse.add_argument('-dim-feedforward', type=int, default=32,
                       help='hidden layer dimension in feedforward layer')
    ...
    parse.add_argument('-batch-size', type=int, default=128,
                       help='number of samples in a batch')  # batch
    parse.add_argument('-epoch', type=int, default=200, help='number of iteration')  # epoch
    parse.add_argument('-k-fold', type=int, default=-1,
                       help='k in cross validation,-1 represents train-test approach')
